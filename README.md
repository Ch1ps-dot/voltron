...
- `results-<target>-voltron-<timestamp>/diagnostics/`: runtime diagnostics,
  grouped as `logs/`, `status/`, `events/`, and `summary/`
...
- `results-<target>-voltron-<timestamp>/generator_iteration_metrics.csv`:
  ...
  mutation rounds
- `results-<target>-voltron-<timestamp>/iteration_state_metrics.csv`:
  one response-graph snapshot per model-learning or Berserker iteration
- `compliance_analysis/`: optional per-pair protocol compliance judgments
  generated by `analyze_compliance.py`

Runtime logs are written to `diagnostics/logs/fuzz.log` and
`diagnostics/logs/llm.log`. The latest `fuzzer_status` and final
`run_status.json` are in `diagnostics/status/`; append-only lifecycle, LLM
validation, and response-component-use records are in `diagnostics/events/`.
The component summary is in `diagnostics/summary/`. Crash reports and
invalid-response evidence remain in their dedicated result directories because
they are replay and post-processing artifacts rather than ordinary diagnostics.
...
`iteration_state_metrics.csv` is the cross-phase iteration series. Its
`nodes` and `edges` use the same response-graph definitions as `states.csv`,
while `iteration_nodes` and `iteration_edges` record only observations from
that completed outer iteration. It also retains lifetime node/edge counts for
whole-run trend analysis.
