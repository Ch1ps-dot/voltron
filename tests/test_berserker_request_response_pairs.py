...
def test_skips_abnormal_empty_and_incomplete_exchanges(tmp_path):
    ...
    berserker.analyze_cons(empty_request, trans_inc=0, type_inc=0)

    incomplete = Conversation()
    incomplete.res_seq.append('230')
    berserker.analyze_cons(incomplete, trans_inc=0, type_inc=0)

    abnormal = Conversation()
    add_exchange(abnormal, 'USER', 'TIMEOUT', b'USER test\r\n', b'')
    berserker.analyze_cons(abnormal, trans_inc=0, type_inc=0)
